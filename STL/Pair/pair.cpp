...
int main()
{
    ...
    pair<int, int> p; // build a pair named p
    // Put value on pair
    // First way 
    p = make_pair(1,1);
    // or p = {1,1}
    ...
    // Copy value of Pair
    pair<int, int> p1 = p; // Copy value of Pair
    pair<int, int> &p2 = p; // Reference value of Pair

    // Create a  pair of arrays
    pair<int, int> p3 [4]; // Array of pairs of size 4
    exec(start);
    return 0;
}